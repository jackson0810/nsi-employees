...
from employees.forms import NewsItemForm, FunctionalCapabilityForm, TaskOrderForm, ImageItemForm, FormDataForm, \
    FomCategoryForm
...
def news_items(request, news_uuid=None):
    ...
    if news_uuid:
        try:
            ...
            return redirect('employees:news_items')
...
def functional_capabilities(request, capability_uuid=None):
    ...
    if capability_uuid:
        try:
            ...
            return redirect('employees:functional_capabilities')
...
def contact_items(request, contact_uuid=None):
    ...
    if contact_uuid:
        try:
            ...
            return redirect('employees:contact_items')
...
def form_category(request, category_uuid=None):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('employees:form_category')
        else:
            messages.error(request, settings.GENERIC_ERROR)
    else:
